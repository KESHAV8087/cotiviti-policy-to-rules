...
def run_agent(task: str, max_steps: int = 5) -> str:
    ...
    print(">>> POLICY_AGENT run_agent CALLED. Tools:", [t["function"]["name"] for t in TOOL_SCHEMAS])
    ...
    for step in range(1, max_steps + 1):
        ...
        if not tool_calls:
            ...
            if content:
                return content
            # Model returned nothing; explicitly ask for the JSON.
            messages.append({
                "role": "user",
                "content": "Now output ONLY the JSON array of rules, nothing else.",
            })
            continue
